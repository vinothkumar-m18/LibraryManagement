...
/**
 * Represents a class responsible for handling user input and interaction with other classes to perform appropriate operations based on the user's input.
 */
public class UserInput {
  private Scanner scanner;
  private User user;
  // Below variables are passed from librarian class, and not instantiated within this class.
  private FileManager fileManager; 
  private Librarian librarian;

  /**
   * Creates a instance of the UserInput class and assigning necessary resources such as FileManager and Librarian reference to the instance variables of the UserInput class.
  ...
  /**
   * Evalauates whether a gmail ID is valid by checking some conditions such as thee presence of spaces, special characters, and ensuring the gmail ID is not empty.
   * @param gmailID the gmail ID of the user to evaluate
   * @return true if the gmail ID is not empty, doesn't contain spaces or any special characters other than (.) and (@); false otherwise
   */
  ...
  /**
   * Evaluates a string to determine if it contains a special character based on its meta data type.
   * (e.g., username, password, gmail).
   * @param string the string to evaluate
   * @param metaData the meta data about the string type(e.g., "USERNAME", "PASSWORD", "GMAIL")
   * @return true if the string contains any special character by its metadata; false otherwise
   */
    ...
    for (int i = 0; i < array.length; i++) {
      switch (metaData) {
        case "USERNAME":
          ...
          if ((array[i] >= 0 && array[i] <= 31) || (array[i] >=33 && array[i] <= 47)|| (array[i] >= 58 && array[i] <= 64)
        ...
        case "PASSWORD":
          ...
          break;
        case "GMAIL":
          if ((array[i] >= 0 && array[i] <= 31) ||(array[i] >= 33 && (array[i] <= 45) || array[i] == 47)|| (array[i] >= 58 && array[i] <= 63)
              || (array[i] >= 91 && array[i] <= 96) || (array[i] >= 123 && array[i] <= 126)){
                return true;
          }
            break;
  ...
  /**
   * Evaluates a password based on some conditions, such as containing a uppercase letter, a lowercase letter, a special character, and a digit.
  ...
  /**
   * Validates whether a stock count is valid by ensuring that its neither zero, negative, nor empty.
  ...
  /**
   * Evaluates a username based on conditions such as ensuring it doesn't contain digits or special characters other than (.) and space.
  ...
  private void exitMessage() {
    fileManager.writeAllBooksToDisk("Books.txt");
